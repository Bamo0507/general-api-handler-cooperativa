...
fn test_get_all_payments_returns_all_inserted_payments() {
    ...
    for expected in expected_sorted.iter() {
        ...
        assert_eq!(
            expected.being_payed, actual.being_payed,
            "being_payed should match"
        );
        // Como no insertamos users:{hash}:complete_name en el test, presented_by_name debe ser "N/A"
        assert_eq!(
            actual.presented_by_name, "N/A",
            "presented_by_name should be N/A when no user data exists"
        );
    }
}
...
fn test_get_user_payments_returns_only_user_payments() {
    ...
    // Insertar nombre del usuario en Redis
    {
        ...
        let _: () = con
            .set(&name_key, "Test User Payments")
    ...
    // Insertar pagos en Redis
    {
        ...
        let key1 = format!(
            "users:{}:payments:{}",
            db_access_token, redis_payment1.ticket_number
        );
        let key2 = format!(
            "users:{}:payments:{}",
            db_access_token, redis_payment2.ticket_number
        );

        let _: () = con
            .json_set(&key1, "$", &redis_payment1)
            .expect("Failed to insert payment 1");
        let _: () = con
            .json_set(&key2, "$", &redis_payment2)
    ...
    let result = repo
        .get_user_payments(test_access_token.to_string())
    ...
    let payment1 = result
        .iter()
        .find(|p| p.ticket_num == redis_payment1.ticket_number);
    let payment2 = result
        .iter()
        .find(|p| p.ticket_num == redis_payment2.ticket_number);
...
fn test_get_user_history_returns_correct_values() {
    ...
    // Insertar valores de historial en Redis
    {
        ...
        let _: () = con
            .set(&payed_key, payed_to_capital_value.to_string())
            .expect("Failed to set payed_to_capital");
        let _: () = con
            .set(&owed_key, owed_capital_value.to_string())
    ...
    let result = repo
        .get_user_history(test_access_token.to_string())
        .expect("get_user_history failed");

    // Validaciones
    assert_eq!(
        result.payed_to_capital, payed_to_capital_value,
        "payed_to_capital debería ser {}",
        payed_to_capital_value
    );
    assert_eq!(
        result.owed_capital, owed_capital_value,
        "owed_capital debería ser {}",
        owed_capital_value
    );
}
...
fn test_get_user_history_with_no_data_returns_error() {
    ...
    assert!(
        result.is_err(),
        "Debería retornar error cuando no hay datos de historial"
    );
