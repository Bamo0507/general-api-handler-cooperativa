...
fn test_rechazar_pago_pendiente_sin_comentario() {
    ...
    assert_eq!(
        result.unwrap_err(),
        "Se requiere comentario al rechazar el pago"
    );
}
...
fn test_mutar_con_estado_invalido() {
    ...
    assert_eq!(
        result.unwrap_err(),
        "Estado inválido, debe ser ACCEPTED o REJECTED"
    );
}
