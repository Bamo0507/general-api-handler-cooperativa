...
use crate::{models::auth::SignUpInfo, repos::auth::create_user_with_access_token};
...
pub async fn user_sign_up(user_data: web::Json<SignUpInfo>) -> HttpResponse {
    ...
    return HttpResponse::Ok().json(create_user_with_access_token(
        data.user_name.to_string().clone(),
        data.pass_code.to_string().clone(),
        data.real_name.to_string().clone(),
    ));
}

//This will be used on mobile prod
pub async fn user_login(data: web::Json<SignUpInfo>) -> HttpResponse {
    unimplemented!();
}
